...
class MOVIE_GAN():
    def cords_to_map(self, cords, img_size, sigma=6):
        ...
        for i in range(cords.shape[0]):
            if cords[i][0] == MISSING_VALUE or cords[i][1] == MISSING_VALUE:
                continue
            xx, yy = np.meshgrid(np.arange(img_size[1]), np.arange(img_size[0]))
            result += np.exp(-((yy - cords[i][0]) ** 2 + (xx - cords[i][1]) ** 2) / (2 * sigma ** 2))
        return result
    ...
    def __init__(self):
        ...
        #error
        img = self.cords_to_map(poses, self.img_size)
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(8, activation="relu", kernel_size=3, strides=2, input_shape=self.img_size, padding="same"))
        model.add(Dropout(0.25))
        model.add(Conv2D(16, activation="relu", kernel_size=3, strides=2, padding="same"))
        model.add(ZeroPadding2D(padding=((0,1),(0,1))))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dropout(0.25))
        model.add(Conv2D(32, activation="relu", kernel_size=3, strides=2, padding="same"))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dropout(0.25))
        model.add(Conv2D(64, activation="relu", kernel_size=3, strides=2, padding="same"))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dropout(0.25))
        model.add(Flatten())
    ...
    def train(self, epochs, batch_size=32, save_interval=50):
        ...
        img_gen = np.zeros(batch_size + self.img_size)
        img_train = np.zeros(batch_size + self.img_size)
        ...
        for epoch in range(epochs):
            ...
            for i in range(batch_size):
                ...
                img_train[i] = self.cords_to_map(pose_man_train[i], self.img_size)
            ...
            for i in range(batch_size):
                img_gen[i] = self.cords_to_map(pose_man_gen[i], self.img_size)

            # Train the discriminator (real classified as ones and generated as zeros)
            d_loss_real = self.discriminator.train_on_batch(img_train, valid)#valid=real
            d_loss_fake = self.discriminator.train_on_batch(img_gen, fake)#fake
...
movie_gan.train(epochs=3, batch_size=1, save_interval=1000)
K.clear_session()
