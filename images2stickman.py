...
if __name__ == "__main__":
    ...
    count = 0
    input_folder = './dance_images/'
    output_folder = './dance_annotations/'
    ...
    num1 = len(input_list)
    for i in range (num1):
        sub_folder  = '%s/' % os.listdir(input_folder)[i]
        output_path = output_folder + input_list[i] + '.csv'
        ...
        images_list = os.listdir(input_folder + sub_folder)
        num2 = len(images_list)
        for j in range (num2):
            image_name = images_list[j]

            if image_name in processed_names:
                continue

            pose_cords = cordinates_from_image_file(input_folder + sub_folder + image_name, model=model)
...
"""
            sample_img = cv2.imread(input_folder + sub_folder + image_name)

            for l in range(18):
                 sample_img[pose_cords[l,0]][pose_codes[l,1]][:] = (0,0,0)

            cv2.imwrite('%s.png'% str(j),sample_img)
"""
