...
class MOVIE_GAN():
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(16, kernel_size=(3, 4), strides=(1, 2), padding='same', input_shape=self.pose_body_movie_shape))
        model.add(BatchNormalization())
        model.add(Activation("relu"))
        model.add(Flatten())
        ...
        pose_movie = Input(shape=self.pose_body_movie_shape)
    ...
    def train(self, epochs, batch_size=32, save_interval=50):
        ...
        train_b = np.zeros((len(annotation_list), ) + self.pose_body_movie_shape)
...
#train data fitting
        ...
        for i in range(len(annotation_list)):
            for j in range(self.flames):
                for k in range(self.body_annotations):
                    for l in range(self.coordinates):

                        if train_b[i,j,k,l] == -1:
                            if j == 0:
                                train_b[i,j,k,l] = (train_b[i,j+1,k,l] + train_b[i,j+2,k,l])/2
                            elif not j == self.flames - 1:
                                train_b[i,j,k,l] = (train_b[i,j-1,k,l] + train_b[i,j+1,k,l])/2
                            else:
                                train_b[i,j,k,l] = (train_b[i,j-1,k,l] + train_b[i,j-2,k,l])/2

        train_b = train_b / 127.5 - 1
        ...
        for epoch in range(epochs):
            ...
            # If at save interval => save generatemovie-body_gand image samples
    ...
    def save_annotations(self, epoch):
        ...
        pose_movie = np.zeros((r * c, ) + self.pose_movie_shape)
        pose_movie -= 1
        ...
        for i in range(r * c):
            ...
            for t in range(32):
                print('%s.jpg: %s: %s' % ('{0:02d}'.format(t), str(list(pose_movie[i, t, :, 0])), str(list(pose_movie[i, t, :, 1]))), file=result_file)
                result_file.flush()
